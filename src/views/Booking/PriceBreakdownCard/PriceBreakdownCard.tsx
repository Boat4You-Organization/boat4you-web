  ...
  return (
    <Box className={cx(styles.container, { [styles.compact]: showCard, [styles.lastStep]: isLastStep })}>
      ...
      {/* Payment schedule — timeline with dots + connector (same as cancellation),
          ...
          Source priority: partner-aware phases passed as a prop (preview from
          /public/reservations/payment-phases-preview, or post-reservation
          actual phases) → client-side A/B/C fallback only when neither is
          available (offline, network error). */}
      {dateFrom && totalPriceEur > 0 && (
        <>
          ...
          {isLoadingPhases && paymentPhases.length === 0 ? (
            ...
            <Stack>
              {(paymentPhases.length > 0
                ? paymentPhases.map(p => ({
                    // Backend returns ISO date strings; the local fallback returns
                    // Dayjs objects. Normalize to Dayjs here so the formatter and
                    // both branches behave identically.
                    deadline: DateTime.date(p.deadline),
                    amount: p.amount,
                  }))
                ...
                const amount = formatPriceWithCurrency({
                  ...
                  clientPriceInfo: totalPriceInfo
                    ? {
                        amount: (phase.amount / totalPriceEur) * totalPriceInfo.amount,
