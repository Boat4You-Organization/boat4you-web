...
const AmenitiesTab = ({ reservationDetails }: AmenitiesTabProps) => {
  ...
  const renderAmenitiesList = (category: YachtEquipmentCategoryType) => {
    const filtered =
      reservationDetails.amenities?.filter(
        (amenity: YachtAmenitiesModel) => amenity.equipment?.category === category,
      ) || [];
  ...
  const hasAmenities = reservationDetails.amenities && reservationDetails.amenities.length > 0;
